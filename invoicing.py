...
def next_invoice_number(invoice_number: str) -> str:
    ...
    """
    current_year, number = parse_invoice_number(invoice_number)
...
def record_invoice(invoice_file: TextIO,
    ...
    if last_row:
        ...
        invoice_number =next_invoice_number(invoice_number)

    else:
        invoice_year = get_year()
        number = "1"
        invoice_number = f"{invoice_year}-{number.zfill(4)}"

    last_line_ptr = invoice_file.tell()
    print(f"{invoice_number}\t{company}\t{amount}", file=invoice_file)
    return last_line_ptr


with open("invoices.csv", "r+") as invoice_file:
    last_line = record_invoice(invoice_file, 'ACME Roadrunner', 18.40)
    record_invoice(invoice_file, 'Squirrel Storage', 320.55, last_line)
